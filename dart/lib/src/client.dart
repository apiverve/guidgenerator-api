...
class GuidgeneratorValidationException implements Exception {
  final List<String> errors;

  GuidgeneratorValidationException(this.errors);

  @override
  String toString() => 'GuidgeneratorValidationException: ${errors.join("; ")}';
}
...
/// GUID Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/guidgenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [count] - Number of GUIDs to generate [min: 1, max: 100]
/// * [format] - Output format
class GuidgeneratorClient {
  final String apiKey;
  ...
  GuidgeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/guidgenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [GuidgeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw GuidgeneratorValidationException(errors);
    }
  }

  /// Execute a request to the GUID Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<GuidgeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw GuidgeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return GuidgeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw GuidgeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw GuidgeneratorException('Resource not found');
      } else {
        throw GuidgeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is GuidgeneratorException) rethrow;
      throw GuidgeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the GUID Generator API client.
class GuidgeneratorException implements Exception {
  final String message;

  GuidgeneratorException(this.message);

  @override
  String toString() => 'GuidgeneratorException: $message';
}
